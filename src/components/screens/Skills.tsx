        ...
        <div className="flex flex-col gap-5 sm:gap-6 w-full">
          {categories.map((category, idx) => (
            <motion.div
              ...
              className="bg-slate-900/40 border border-teal-500/20 rounded-3xl backdrop-blur-2xl shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] hover:border-teal-400/50 transition-all duration-300 flex flex-col lg:flex-row lg:items-center justify-between gap-6 p-5 sm:p-7 lg:p-8"
            >
              ...
              <div className="flex items-center gap-3.5 lg:w-1/3 border-b lg:border-b-0 lg:border-r border-white/10 pb-4 lg:pb-0 lg:pr-6 shrink-0">
                <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-2xl bg-teal-400/10 border border-teal-400/30 flex items-center justify-center shrink-0">
                  {category.icon}
                </div>
              ...
              {/* Skills Wrap Area */}
              <div className="flex flex-wrap items-center gap-2.5 sm:gap-3 lg:w-2/3">
                {category.skills.map((skill, index) => (
                  <div
                    key={index}
                    className="inline-flex items-center gap-2.5 rounded-xl bg-teal-400/10 border border-teal-400/25 text-teal-200 hover:text-white hover:bg-teal-400/20 hover:border-teal-400/60 hover:shadow-[0_0_12px_rgba(45,212,191,0.3)] transition-all duration-300 cursor-default whitespace-nowrap px-3.5 py-2 sm:px-4 sm:py-2.5"
                  >
                    <span className="text-base sm:text-lg text-teal-300 shrink-0">
                      {skill.icon}
                    </span>
